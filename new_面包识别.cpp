...
{
	MultiTracker myTracker;  //利用"tracking.hpp"里的多目标跟踪类
...
/*		if(trackObject<0){ //添加第一个面包进入跟踪
		...
		if(trackObject<0){
			...
			for(int i=n; i<breadNum; i++){
				Point2f CM = breads[i].getCenter();
				float sideLenth = breads[i].getRadius()*2;
				Rect2d selection(CM.x-sideLenth*0.5, CM.y-sideLenth*0.5, sideLenth, sideLenth);
				obj.push_back(selection);
				algorithms.push_back(TrackerKCF::create());
			}
			myTracker.add(algorithms, frame, obj);
			trackObject = 1;
		}
